        ...
        if len(self.price_data) > 2:
            if strat == 'opt':
                ...
                self.opt = self.MaximizeSharpeRatioOptmzn(
                    self.ind_er.to_numpy(), self.cov_matrix.to_numpy(), 0.0, 9)
                return self.opt.x
                ...
                self.calculate_matrices()
                self.opt = self.MaximizeSharpeRatioOptmzn(
                    self.ind_er.to_numpy(), self.cov_matrix.to_numpy(), 0, 9)
                ...
                weights = self.opt.x
                ind = np.argpartition(
                    weights, -self.num_elems)[-self.num_elems:]
                new_weights = []
                for i in range(self.num_elems):
                    if i in ind:
                        new_weights.append(weights[i])
...
def strat_function(preds, prices, last_weights):
    ...
    opt = strat.update_weights(strat='opt')
